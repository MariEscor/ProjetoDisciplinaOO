...
class Npc(Entidades):
    ...
    """
    def __init__(self, pos: tuple[int, int], frames: Dict[str, List[pygame.Surface]], groups: pygame.sprite.Group, olhando: str, npc_dados: Dict[str, Any], player: 'Player',
        cria_dialogo: Callable[['Npc'], None], inicia_batalha: Callable[['Npc'], None], colisao_sprites: pygame.sprite.Group, raio: int) -> None:
        ...
        self.__inicia_batalha: Callable[['Npc'], None] = inicia_batalha  # ✅ Passado via parâmetro

        self.__colisao_rects: List[pygame.Rect] = [
            sprite.rect for sprite in colisao_sprites if sprite is not self
        ]

        self.__pode_mover: bool = False
        self.__pode_girar: bool = True
        self.__notou_player: bool = False

        self.__raio: int = int(raio)
        self.__visao_direcoes: List[str] = npc_dados['directions']

        self.__timers: Dict[str, Timer] = {
            'look_around': Timer(1500, autostart=True, repete=True, funcao=self.olhada_aleatoria),
            'notice': Timer(500, funcao=self.comeca_mover)
        }
    ...
    @property
    def inicia_batalha(self) -> Callable[['Npc'], None]:
        return self.__inicia_batalha

    @inicia_batalha.setter
    def inicia_batalha(self, value: Callable[['Npc'], None]) -> None:
        if not callable(value):
            raise TypeError("A 'inicia_batalha' deve ser uma função de callback.")
        self.__inicia_batalha = value
    ...
    def mexendo(self, dt: float) -> None:
        if not self.pode_mover and self.direcao:
            ...
            if not self.hitbox.inflate(10, 10).colliderect(self.player.hitbox):
                ...
                if not self.npc_dados.get("defeated", False):
                    self.inicia_batalha(self)
                else:
                    self.cria_dialogo(self)
                # Chama a função de callback para criar o diálogo
                self.player.notificacao = False # Desativa a notificação visual do jogador
        else:
            # Adiciona diálogo caso já esteja colado, mas defeated
            if self.npc_dados.get("defeated", False) and self.pode_mover:
                self.cria_dialogo(self)
