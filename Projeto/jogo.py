...
import Projeto.player_state
from bag_manager import get_selected_pokemons
from battle_scene import run_battle
from battle_context import rival_pokemon, player_team
...
from bag_manager import add_pokemon_to_bag, get_selected_pokemons
...
class Jogo:
        ...
        for obj in tmx_mapa.get_layer_by_name('Entidades'):
            ...
            if obj.name == 'Player':
                if player_ini_pos == "dinamica":
                    self.player = Player(
                        pos = Projeto.player_state.player_state["pos"],
                        frames = self.mapa_frames['characters']['player'],
                        groups = self.all_sprites,
                        olhando = Projeto.player_state.player_state["direction"],
                ...
                #NPCs
                #se for copiado do original, não precisa adicionar 'direction' e o mesmo vale para character_id 
                Npc(
                    pos = (obj.x, obj.y), 
                    frames = self.mapa_frames['characters'][obj.properties['graphic']], 
                    groups = (self.all_sprites, self.colisao_sprites, self.npc_sprites),
                    olhando = obj.properties['direction'],
                    npc_dados = TRAINER_DATA[obj.properties['character_id']],
                    player = self.player,
                    cria_dialogo = self.cria_dialogo,
                    colisao_sprites = self.colisao_sprites,
                    raio = obj.properties['radius'] #propriedade do tmx radius
                    #adicionar depois
                    #medica
                    #notificacao_som
                ) 
    ...
    def cria_dialogo(self, character: Any) -> None:
        if not self.arvore_dialogo:
            self.arvore_dialogo = ArvoreDeDialogo(character, self.player, self.all_sprites, self.fonts, self.finaliza_dialogo)

    #character é passado em arvore_dialogo.py, que é o npc que está sendo interagido
    def finaliza_dialogo(self, character: Any) -> None:
            ...
            if area.rect.colliderect(player_hitbox):
                if random.random() < 0.20:  # 20% de chance
                    ...
                    if pokemon_inimigo:
                        ...
                        player_team[:] = get_selected_pokemons()
                        rival_pokemon = pokemon_inimigo

                        # Salva posição atual do jogador
                        Projeto.player_state.player_state["map"] = self.transition_target[0] if self.transition_target else "mundo"
                        Projeto.player_state.player_state["pos"] = self.player.rect.topleft
                        Projeto.player_state.player_state["direction"] = self.player.olhando
