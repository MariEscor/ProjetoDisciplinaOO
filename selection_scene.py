...
class SelectionScene(GameScene):
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        self._ui.draw_text(screen, 'Escolha seu Pokémon Inicial!', 42, constants.BLACK, (constants.LARGURA_JANELA // 2, 80))
        ...
        while True:
            ...
            if getattr(scene, "next_scene", None) == "battle" and not pokemon_salvo:
                if scene.selected_pokemon:
                    # CORREÇÃO: usar o bag_manager de instância
                    scene._bag_manager.set_initial_pokemon(scene.selected_pokemon.name)
                    pokemon_salvo = True
                    timer.game_start_time = time.time()
                    scene._scene_manager.load_scene("mundo_aberto")
                break
